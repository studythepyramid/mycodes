...
def ccsequences(data, window_size, prediction_steps, features, label):
    print(f"wiindow size: {window_size}, prediction steps: {prediction_steps}")
...
enumed = enumerate(train_dataloader)
enumedList = list(enumed)
print(len(enumedList))
enumedList[0]

batch_idx, (inputs, targets) = enumedList[0]
